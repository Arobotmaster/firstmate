...
fm_control_harnesses() {
  printf '%s\n' claude mirasim codex opencode pi pi-signed grok kimi cursor gemini muse rovo omp agy devin
}
...
# Which task kinds an adapter is verified to run. mirasim, muse, gemini, rovo, agy, and devin
...
fm_control_harness_supports_kind() {  # <harness> <kind>
  ...
  case "$harness" in
    mirasim|muse|gemini|rovo|agy|devin) [ "$kind" != secondmate ] || return 1 ;;
  esac
  return 0
}
...
fm_control_interrupt_key() {  # <harness>
  case "${1-}" in
    claude|mirasim|codex|opencode|pi|pi-signed|omp|kimi|cursor|gemini|muse|rovo|agy|devin) printf 'Escape' ;;
    grok) printf 'C-c' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_repeat() {  # <harness>
  case "${1-}" in
    ...
    claude|mirasim|codex|pi|pi-signed|omp|grok|kimi|cursor|gemini|muse|rovo|agy) printf '1' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
    claude|mirasim|codex|opencode|pi|pi-signed|omp|grok|kimi|cursor|gemini|rovo|agy|devin) ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    claude|mirasim|codex|opencode|pi|pi-signed|omp|grok|kimi|cursor|gemini|rovo|agy|devin) printf 'none' ;;
    *) return 1 ;;
  esac
}
...
fm_control_exit_command() {  # <harness>
  case "${1-}" in
    claude|mirasim|opencode|grok|kimi|cursor|muse|rovo) printf '/exit' ;;
