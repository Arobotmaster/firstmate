...
It is not deterministic across the verified adapters: codex, grok, gemini, and devin resume only from a session id printed at exit, opencode continues the most recent session for the cwd, and claude, mirasim, pi, pi-signed, omp, kimi, and agy have no verified pane-resume contract.
