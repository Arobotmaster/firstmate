...
It classifies recognized Claude, Mirasim, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, Rovo, AGY, and Devin process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
