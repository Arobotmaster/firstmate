...
`bin/fm-harness-adapter-path-check.sh` validates code-marked owner paths in nested harness-adapter references. `tests/fm-harness-adapter-references.test.sh` checks valid and missing path behavior through that command, then parses the router's JSON contract and proves that every selected reference is readable. These checks provide structural evidence only.
