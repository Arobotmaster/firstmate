...
test_mirasim_and_devin_coexist() {
  local harness guards
  for harness in mirasim devin; do
    fm_control_harness_supported "$harness" || fail "$harness disappeared from the registry"
    [ "$(fm_control_harness_family "$harness")" = "$harness" ] || fail "$harness lost its control identity"
    [ "$(fm_agent_process_classify_name "/opt/bin/$harness")" = agent ] || fail "$harness lost liveness classification"
    [ "$(fm_agent_process_classify_name "$harness-helper")" = other ] || fail "$harness claims unrelated processes"
  done
  [ "$(fm_control_interrupt_repeat mirasim)" = 1 ] || fail "Mirasim must retain Claude's single Escape"
  [ "$(fm_control_interrupt_repeat devin)" = 2 ] || fail "Devin must retain its guarded double Escape"
  [ "$(fm_control_exit_command devin)" = /quit ] || fail "Devin must retain /quit"
  fm_busy_source_trusted mirasim claude-hook || fail "Mirasim hook ownership changed"
  fm_busy_source_trusted devin devin-hook || fail "Devin hook ownership changed"
  if fm_busy_source_trusted mirasim devin-hook || fm_busy_source_trusted devin claude-hook; then
    fail "Mirasim and Devin must not accept each other's hook source"
  fi
  guards=$("$ROOT/bin/fm-test-run.sh" --list --family live-harness-optin) || fail "live guard inventory failed"
  for harness in fm-mirasim-live-e2e fm-devin-signals-live-e2e fm-launch-prompt-signals-live-e2e; do
    assert_contains "$guards" "tests/$harness.test.sh" "$harness was lost from the live guard family"
  done
  pass "Mirasim and Devin retain separate lifecycle contracts and the launch-prompt live guard"
}

test_mirasim_reference_paths() {
  python3 - "$ROOT" <<'PY' || fail "Mirasim reference paths do not resolve inside the repository"
import re
import sys
from pathlib import Path

root = Path(sys.argv[1]).resolve()
skill = root / ".agents/skills/harness-adapters"
reference = skill / "references/harness/mirasim.md"
text = reference.read_text()
paths = [(skill, value) for value in re.findall(r"`([^`\n]+)`", text) if value.startswith("../")]
paths += [(reference.parent, value) for value in re.findall(r"\]\(([^)]+)\)", text) if value.startswith("../")]
assert paths, "Mirasim reference has no owner paths"
for base, value in paths:
    target = (base / value).resolve()
    target.relative_to(root)
    assert target.is_file(), f"unreadable Mirasim owner: {value}"
PY
  pass "Mirasim owner paths use the skill root and Markdown targets use the reference directory"
}
