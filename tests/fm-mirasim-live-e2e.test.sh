...
open_cmd="'$ROOT/bin/fm-busy-event.sh' apply '$STATE' '$ID' busy --gen '$gen' --source claude-hook --event user-prompt-submit && touch '$STATE/$ID.submitted'"
close_cmd="'$ROOT/bin/fm-busy-event.sh' apply '$STATE' '$ID' idle --gen '$gen' --source claude-hook --event stop && touch '$STATE/$ID.turn-ended'"
jq -n --arg open "$open_cmd" --arg close "$close_cmd" '{hooks:{UserPromptSubmit:[{hooks:[{type:"command",command:$open}]}],Stop:[{hooks:[{type:"command",command:$close}]}]}}' \
...
[ -f "$STATE/$ID.submitted" ] \
  || fail "the real Mirasim turn did not publish UserPromptSubmit"
